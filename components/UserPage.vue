    ...
    <div>
      ...
      <main>
        <nav>
          ...
          <ul class="data-list">
            <li @click="showFollowingList" :class="{ active: active === 3 }">
              关注数 {{ user.views }}
            </li>
        ...
        <article>
          ...
          <div v-else class="list">
            <video-item v-if="portfolio.length>0" v-for="item in portfolio" :key="item" :config="item"/>
...
<script lang="ts" setup>
  ...
  // 获取用户的视频数据
  let res = await getVideo({authorEmail:user.email});
  ...
  const showFollowingList = async () => {
    active.value = 3;
    let res = await getFollowing({userId:user.id});
    upList.value = res.data;
    console.log(upList.value,'关注列表');
